...
def parser():
    ...
    return K_u,N,K_e,Pu,period,GPU


def main(argv):
    ## global library
    
    K_u, N, K_e, P_u, simulation_max, GPU = parser()
    ...
    # initial zeros
    adapt_anal_d = np.zeros(len(K_s),dtype=float)
    adapt_simu_d = np.zeros(len(K_s),dtype=float)
    adapt_anal_e = np.zeros(len(K_s),dtype=float)
    adapt_simu_e = np.zeros(len(K_s),dtype=float)
    ...
    for K_s_index in range(len(K_s)):
        ...
        adapt_anal_d[K_s_index] = 1 - sum_anal_throughput_d
        adapt_anal_e[K_s_index] = 1 - sum_anal_throughput_e
        
        print('K_s= ' + str(np.around(K_s[K_s_index],1))
                + ' Outage_Anal_D= ' + str(np.around(adapt_anal_d[K_s_index],2))
                + ' Outage_Anal_E= ' + str(np.around(adapt_anal_e[K_s_index],2))
                ,end='\n')
        ...
        while(1):
            ...
            H_sr = np.reshape(channel_vector(K_s[K_s_index],K_u,N,1,'rayleigh'),(N,K_u,K_s[K_s_index]))
            ...
            if relay_counter == 0:
                ...
                sum_H_re = np.sum(H_re.reshape(K_e,int(K_u*relay_counter)),1)
            ...
            print('\r' 
                + 'K_s= ' + str(np.around(K_s[K_s_index],1)) 
            ...
            if np.any([
                ...
                adapt_simu_d[K_s_index] = adapt_d_counter / simulation_time
                adapt_simu_e[K_s_index] = adapt_e_counter / simulation_time
                print('\n', end='')
                break

    # make dir if not exist
    directory = 'result_txts/ks/adapt/K_u=' + str(K_u)  + '_N=' + str(N)  + '/'
    try:
