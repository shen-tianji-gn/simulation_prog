...
def main(argv):
    ...
    # initial zeros
    adapt_anal_d = np.zeros(len(K_u),dtype=float)
    adapt_simu_d = np.zeros(len(K_u),dtype=float)
    adapt_anal_e = np.zeros(len(K_u),dtype=float)
    ...
    for K_u_index in range(len(K_u)):
        ...
        for n in range(N+1):
            ...
            if n > 0:
                ...
                r_d_throughput = 1 - outage_ud_adapt(K_u[K_u_index],
                                                      r_s,
                                                      zeta,
                                                      sigma_e,
                                                      n,
        ...
        adapt_anal_d[K_u_index] = 1 - sum_anal_throughput_d
        adapt_anal_e[K_u_index] = 1 - sum_anal_throughput_e
        
        print('K_u= ' + str(np.around(K_u[K_u_index],1))
                + ' Outage_Anal_D= ' + str(np.around(adapt_anal_d[K_u_index],2))
                + ' Outage_Anal_E= ' + str(np.around(adapt_anal_e[K_u_index],2))
                ,end='\n')
        ...
        while(1):
            ...
            if relay_counter == 0:
                ...
                sum_H_re = np.sum(H_re.reshape(K_e,int(K_u[K_u_index]*relay_counter)),1)
                ...
                R_d = (1 - zeta) * np.log2(1 + p_u / K_u[K_u_index] * sum_h_rd_2 * alpha_ud ** (-1) 
                                           / (p_u / K_u[K_u_index] * sum_h_ud_err_2 * alpha_ud ** (-1) + sigma))
                R_e = (1 - zeta) * np.log2(1 + p_u / K_u[K_u_index] * sum_H_re_2 * alpha_ue ** (-1)
                                           / (p_u / K_u[K_u_index] * sum_H_je_2 * alpha_ue ** (-1) + sigma))
    ...
    directory = 'result_txts/ku/adapt/K_s=' + str(K_s)  + '_N=' + str(N)  + '/'
    try:
