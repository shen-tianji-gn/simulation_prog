...
def parser():
    usage = 'Usage: python {} [--Ku <Number of Ku>] [--Ke <Number of Ke>] [--Pu <Output Power of Device (dBm)>] [--Period <Simulation period>] [--GPU <Simulation GPU ID>] [--help]'
    ...
    return K_u,K_s,K_e,Pu,period,GPU


def main(argv):
    ## global library
    
    K_u, K_s, K_e, P_u, simulation_max,GPU = parser()
    ...
    N_min = par_lib.N_min
    N_max = par_lib.N_max 
    N_inst = par_lib.N_inst
    N = np.arange(N_min,N_max+N_inst,N_inst)


    # initial zeros
    adapt_anal_d = np.zeros(len(N),dtype=float)
    adapt_simu_d = np.zeros(len(N),dtype=float)
    adapt_anal_e = np.zeros(len(N),dtype=float)
    adapt_simu_e = np.zeros(len(N),dtype=float)
    ...
    for N_index in range(len(N)):
        ...
        for n in range(N[N_index]+1):
            ...
            if n > 0:
                ...
                r_e_throughput = 1 - outage_ue_adapt(K_e,
                                                     ...
                                                     zeta)
                # outage D
                sum_anal_throughput_d += comb(N[N_index],n) * s_r_throughput ** n * (1 - s_r_throughput) ** (N[N_index]-n) * (r_d_throughput)
                
                # outage E
                sum_anal_throughput_e += comb(N[N_index],n) * s_r_throughput ** n * (1 - s_r_throughput) ** (N[N_index]-n) * (r_e_throughput)
                
    
        adapt_anal_d[N_index] = 1 - sum_anal_throughput_d
        adapt_anal_e[N_index] = 1 - sum_anal_throughput_e
        
        print('N= ' + str(np.around(N[N_index],0))
                + ' Outage_Anal_D= ' + str(np.around(adapt_anal_d[N_index],2))
                + ' Outage_Anal_E= ' + str(np.around(adapt_anal_e[N_index],2))
                ,end='\n')
        ...
        while(1):
            ...
            H_sr = np.reshape(channel_vector(K_s,K_u,N[N_index],1,'rayleigh'),(N[N_index],K_u,K_s))
            ...
            if relay_counter < N[N_index]:
                h_jd_e = estimation_error((K_u * (N[N_index] - relay_counter), 1), sigma_e)
                H_je = channel_vector(K_u,K_e,N[N_index]-relay_counter,0,'rayleigh',GPU_ID=GPU)
            else:
                h_jd_e = 0
                H_je = 0
            ...
            if np.any([
                ...
                adapt_simu_d[N_index] = adapt_d_counter / simulation_time
                adapt_simu_e[N_index] = adapt_e_counter / simulation_time
                print('\n', end='')
                break
    ...
    directory = 'result_txts/N/adapt/K_s=' + str(K_s) + '_K_u='+ str(K_u) + '/'
    try:
    ...
    for _ in range(len(file_path)):
        output(file_path[_],N,len(N),file_results[_])
