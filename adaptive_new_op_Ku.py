...
def parser():
    usage = 'Usage: python {} [--N <Number of N>] [--Ko <Number of Ko>] [--Pu <Output Power of Device (dBm)>] [--help]'
...
# marcumq (calculated by non-central chi-squared cdf in scipy)
def marcumQ(mu,alpha,beta):
    Q = 1 - ncx2.cdf(beta ** 2, mu ** 2, alpha ** 2)
    return Q
...
# bubblesort_matrix
def bobblesort_matrix(mat,location):
    '''
    bobblesort with the matrix
    '''
    x, y = mat.shape
    if location == 'x':
        for i in range(x):
            for j in range(0, x-i-1):
                if np.abs(np.sum(mat,axis=0))[j] > np.abs(np.sum(mat,axis=0))[j+1]:
                    mat[[j,j+1], :] = mat[[j+1,j] :]

    elif location == 'y':
        for i in range(y):
            for j in range(0,y-i-1):
                if np.sum(mat,axis=1)[j] > np.sum(mat,axis=1)[j+1]:
                    mat[:,j], mat[: j+1] = mat[:j+1], mat[:j].copy
...
def reshape_r_matrix(matrix):
    '''
    input: 3d matrix (x,y,z)
    output: matrix (x*z,y)
    '''
    a = np.transpose(matrix,(1,0,2))
    a = np.reshape(a,(3,4)).T
    return a
    ...
    if type == 0:
        ...
        if in_the_sight == 'rayleigh':
            ...
            for n_index in range(N):
                ...
                y = norm.rvs(loc=0,scale=np.sqrt(1/2), size=(Rx_num,Tx_num))
                h_nlos[n_index] = x + 1j * y
                h_los[n_index] = np.exp(-1j * 2 * PI * f * d[n_index]) * np.ones((Rx_num,Tx_num))
            vec_complex = np.reshape(\
                np.sqrt(K / (K + 1)) * h_los + np.sqrt(1 / (K + 1)) * h_nlos,
                (N * Rx_num,Tx_num))
        else:
...
def marcumQ(mu,alpha,beta):
    '''
    Marcum Q function with parameter Q_{mu}(alpha,beta).
    '''
    Q = 1 - ncx2.cdf(beta ** 2, mu * 2, alpha ** 2)
    return Q
...
def main(argv):
    ...
    # initial zeros
    adapt_outage_anal_d = np.zeros(len(K_u),dtype=float)
    adapt_outage_simu_d = np.zeros(len(K_u),dtype=float)
    adapt_capa_anal_d = np.zeros(len(K_u),dtype=float)
    adapt_capa_simu_d = np.zeros(len(K_u),dtype=float)
    adapt_capa_anal_e_best = np.zeros(len(K_u),dtype=float)
    adapt_capa_simu_e_best = np.zeros(len(K_u),dtype=float)
    adapt_capa_anal_e_worst = np.zeros(len(K_u),dtype=float)
    adapt_capa_simu_e_worst = np.zeros(len(K_u),dtype=float)


    adapt_secrecy_anal_best = np.zeros(len(K_u),dtype=float)
    adapt_secrecy_anal_worst = np.zeros(len(K_u),dtype=float)
    adapt_secrecy_simu_best = np.zeros(len(K_u),dtype=float)
    adapt_secrecy_simu_worst = np.zeros(len(K_u),dtype=float)
    ...
    for K_u_index in range(len(K_u)):
        ...
        #     capacity_d = channel_ud_expected(P_u, K_u, K_d, sigma_d, rician_K_D, n, N)
        ...
        #     capacity_e_best = channel_ue_expected(P_u, K_u, K_e, sigma_e, rician_K_E_best, n, N)
        ...
        #     capacity_e_worst = channel_ue_expected(P_u, K_u, K_e, sigma_e, rician_K_E_worst, n, N)
        ...
        # adapt_capa_anal_e_best[P_s_index] = sum_anal_e_best
        # adapt_capa_anal_e_worst[P_s_index] = sum_anal_e_worst
        # adapt_secrecy_anal_best[P_s_index] = np.max([sum_anal_d_cap - sum_anal_e_best,0])
        # adapt_secrecy_anal_worst[P_s_index] = np.max([sum_anal_d_cap - sum_anal_e_worst,0])

        # # print(adapt_capa_anal_e_best[P_s_index])
        # print('Power= ' + str(np.around(P_s[P_s_index],1)) \
        #         + ' adapt_Out_Anal_D= ' + str(adapt_outage_anal_d[P_s_index]) \
        #         + ' adapt_Cap_Anal_E_B= ' + str(adapt_capa_anal_e_best[P_s_index]) \
        #         + ' adapt_Cap_Anal_E_W= ' + str(adapt_capa_anal_e_worst[P_s_index]) \
        #         + ' adapt_Sec_Anal_B= ' + str(adapt_secrecy_anal_best[P_s_index]) \
        #         + ' adapt_Sec_Anal_W=' + str(adapt_secrecy_anal_worst[P_s_index]),
        #         end='\n')
        ...
        while(1):
            ...
            ## pure strategy
            # H_su = np.zeros((int(K_s),int(K_u),int(N)),dtype=complex)
            # H_ud = np.zeros((int(K_u),int(K_d),int(N)),dtype=complex)
            # H_ue_best = np.zeros((int(K_u),int(K_e),int(N)),dtype=complex)
            # H_ue_worst = np.zeros((int(K_u),int(K_e),int(N)),dtype=complex)
            u_state = np.zeros(N,dtype=int)


            # H_su = channel_vector(K_s,K_u,N,1, 'rician', K = rician_K_R, f = frequency, d = dist_u)
            H_su = np.reshape(channel_vector(K_s,K_u[K_u_index],N,1,'rayleigh'),(N,K_u[K_u_index],K_s)) # K_u * K_s
            # H_ud = channel_vector((K_u,K_d,N), 'rician', K = rician_K_D, Omega=Omega)
            # H_ue_best = channel_vector((K_u,K_e,N), 'rayleigh')
            # H_ue_worst = channel_vector((K_u,K_e,N), 'rician', K=rician_K_E_worst, Omega=Omega)
            # H_su_best_sum = np.max(np.abs(np.sum(H_su,axis=0)) ** 2, 0)
            # print(np.sum(H_su,axis=0) ** 2)
            # print(H_su_best_sum)
            # H_su = channel_vector((K_s,K_u,N), 'rayleigh')
            # H_ud = channel_vector((K_u,K_d,N), 'rayleigh')
            # H_ue_best = channel_vector((K_u,K_e,N), 'rayleigh')
            # H_ue_worst = channel_vector((K_u,K_e,N), 'rayleigh')
            ...
            #  K_d * N K_u
            H_ud = channel_vector(K_u[K_u_index],K_d,N,0,'rician', K = rician_K_D, f = frequency, d = dist_u)
            H_ue_best = channel_vector(K_u[K_u_index],K_e,N,0,'rayleigh')
            H_ue_worst = channel_vector(K_u[K_u_index],K_e,N,0,'rician', K = rician_K_E_worst, f = frequency, d = dist_u)
            
            relay_d_matrix = np.zeros((K_d,K_u[K_u_index],N))
            relay_e_matrix = np.zeros((K_e,K_u[K_u_index],N))
            jammer_matrix = np.zeros((K_e,K_u[K_u_index],N))
            ...
            if relay_counter == 0:
                ...
                relay_d_matrix = np.reshape(relay_d_matrix,(K_d,N*K_u[K_u_index]))
                relay_e_matrix = np.reshape(relay_e_matrix,(K_e,N*K_u[K_u_index]))
            ...
            if jammer_counter == 0:
                ...
                for n in range(N):
                    if u_state[n] == 0:
                        jammer_matrix[:,:,n] = np.ones((K_e,K_u[K_u_index]))
                    else:
                        jammer_matrix[:,:,n] = np.zeros((K_e,K_u[K_u_index]))
                
                jammer_matrix = np.reshape(jammer_matrix,(K_e,N*K_u[K_u_index]))
                # print(jammer_matrix)
            ...
            if relay_counter == 0:
                ...
                # print(Lambda_rd_r)
                p_r_optimal = np.zeros((K_u[K_u_index],relay_counter))
                # print(relay_counter)
                # print(Lambda_rd)
        ...
        adapt_outage_simu_d[K_u_index] = adapt_outage_simu_d_counter / simulation_time
        adapt_capa_simu_d[K_u_index] = adapt_capacity_simu_d / simulation_time
        adapt_capa_simu_e_best[K_u_index] = adapt_capacity_simu_e_best / simulation_time
        adapt_capa_simu_e_worst[K_u_index] = adapt_capacity_simu_e_worst / simulation_time
        adapt_secrecy_simu_best[K_u_index] = adapt_sec_capacity_simu_best / simulation_time
        adapt_secrecy_simu_worst[K_u_index] = adapt_sec_capacity_simu_worst / simulation_time
        print('\n', end='')
        
    # make dir if not exist
    try:
        os.makedirs('result_txts/Ku/RicianK=' + str(Rician) + '/adapt_op/N=' + str(N) + '/')
    except FileExistsError:
        pass


    # result output to file
    os.chdir('result_txts/Ku/RicianK=' + str(Rician) + '/adapt_op/N=' + str(N) + '/')
