...
def main(argv):
    ...
    # initial zeros
    adapt_anal_d = np.zeros(len(r_s),dtype=float)
    adapt_simu_d = np.zeros(len(r_s),dtype=float)
    adapt_anal_e = np.zeros(len(r_s),dtype=float)
    adapt_simu_e = np.zeros(len(r_s),dtype=float)
    ...
    for r_s_index in range(len(r_s)):
        ...
        adapt_anal_d[r_s_index] = 1 - sum_anal_throughput_d
        adapt_anal_e[r_s_index] = 1 - sum_anal_throughput_e
        
        print('R_s= ' + str(np.around(r_s[r_s_index],1))
                + ' Outage_Anal_D= ' + str(np.around(adapt_anal_d[r_s_index],2))
                + ' Outage_Anal_E= ' + str(np.around(adapt_anal_e[r_s_index],2))
                ,end='\n')
        ...
        while(1):
            ...
            print('\r' 
                + 'R_s= ' + str(np.around(r_s[r_s_index],1)) 
            ...
            if np.any([
                ...
                adapt_simu_d[r_s_index] = adapt_d_counter / simulation_time
                adapt_simu_e[r_s_index] = adapt_e_counter / simulation_time
                print('\n', end='')
                break
    ...
    directory = 'result_txts/rs/adapt/K_s=' + str(K_s) + '_K_u='+ str(K_u) + '_N=' + str(N)  + '/'
    try:
