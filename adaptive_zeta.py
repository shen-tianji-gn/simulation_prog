...
def main(argv):
    ...
    # initial zeros
    adapt_anal_d = np.zeros(len(zeta),dtype=float)
    adapt_simu_d = np.zeros(len(zeta),dtype=float)
    adapt_anal_e = np.zeros(len(zeta),dtype=float)
    ...
    for zeta_index in range(len(zeta)):
        ...
        adapt_anal_d[zeta_index] = sum_anal_throughput_d
        adapt_anal_e[zeta_index] = sum_anal_throughput_e
        
        print('Zeta= ' + str(np.around(zeta[zeta_index],2))
                + ' Anal_D= ' + str(adapt_anal_d[zeta_index])
                + ' Anal_E= ' + str(adapt_anal_e[zeta_index])
                ,end='\n')
        ...
        while(1):
            ...
            if relay_counter == 0:
                ...
                R_d = (1 - zeta[zeta_index]) * np.log2(1 + p_u / K_u * sum_h_rd_2 * alpha_ud ** (-1) 
                                           / (p_u / K_u * sum_h_ud_err_2 * alpha_ud ** (-1) + sigma))
                R_e = (1 - zeta[zeta_index]) * np.log2(1 + p_u / K_u * sum_H_re_2 * alpha_ue ** (-1)
    ...
    directory = 'result_txts/zeta/adapt/K_s=' + str(K_s)  + '_K_u=' + str(K_u) + '_N=' + str(N)  + '/'
    try:
