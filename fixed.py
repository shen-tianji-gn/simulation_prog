...
def parser():
    ...
    return N,M,K_s,K_u,K_e,Pu,period
...
def main(argv):
    ...
    N, M, K_s, K_u, K_e, P_u, simulation_max = parser()
    K_s = K_s
    K_u = K_u
    K_e = K_e
    ...
    for P_s_index in range(len(P_s)):
        ...
        for n in range(M+1):
            ...
            if n > 0:
                ...
                r_d_throughput = (1 - outage_ud_fix(K_u,r_s,zeta,sigma_e,n,N))
        ...
        print('Power= ' + str(np.around(P_s[P_s_index],1))
                + ' Outage_Anal_D= ' + str(fixed_anal_d[P_s_index])
                + ' Outage_Anal_E= ' + str(fixed_anal_e[P_s_index])
                ,end='\n')
