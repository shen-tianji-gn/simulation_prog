...
def main(argv):
    ...
    # initial zeros
    fixed_anal_d = np.zeros(len(alpha_ue),dtype=float)
    fixed_simu_d = np.zeros(len(alpha_ue),dtype=float)
    fixed_anal_e = np.zeros(len(alpha_ue),dtype=float)
    ...
    for alpha_ue_index in range(len(alpha_ue)):
        ...
        fixed_anal_d[alpha_ue_index] = sum_anal_throughput_d
        fixed_anal_e[alpha_ue_index] = sum_anal_throughput_e
        
        print('Alpha_ue= ' + str(np.around(alpha_ue[alpha_ue_index],2))
                + ' Anal_D= ' + str(fixed_anal_d[alpha_ue_index])
                + ' Anal_E= ' + str(fixed_anal_e[alpha_ue_index])
                ,end='\n')
        ...
        while(1):
            ...
            if relay_counter == 0:
                ...
                # print(R_d)
                R_e = (1 - zeta) * np.log2(1 + p_u / (K_u * alpha_ue[alpha_ue_index]) * sum_H_re_2 \
                    / (p_u / (K_u * alpha_ue[alpha_ue_index]) * sum_H_je_2 + sigma))
            ...
            print('\r' 
                ...
                + ' Simu_D= ' + str(np.around(fixed_d_counter / simulation_time, 3)) 
                + ' Simu_E= ' + str(np.around(fixed_e_counter / simulation_time, 3)) 
    ...
    directory = 'result_txts/alpha_ue/fixed/K_s=' + str(K_s) + '_K_u='+ str(K_u) + '_N=' + str(N) + '_M=' + str(M) + '/'
    try:
