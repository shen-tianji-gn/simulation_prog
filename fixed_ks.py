...
def parser():
    usage = 'Usage: python {} [--Ku <Number of Ku>] [--N <Number of N>] [--M <Number of M>] [--Ke <Number of Ke>] [--Pu <Output Power of Device (dBm)>] [--Period <Simulation period>] [--GPU <Simulation GPU ID>] [--help]'
    ...
    return K_u,N,M,K_e,Pu,period,GPU
...
def main(argv):
    ...
    K_u, N, M, K_e, P_u, simulation_max,GPU = parser()
    ...
    P_s = 0
    K_s_min = par_lib.K_s_min
    K_s_max = par_lib.K_s_max
    K_s_inst = par_lib.K_s_inst
    K_s = np.arange(K_s_min,K_s_max+K_s_inst,K_s_inst)
    ...
    # initial zeros
    fixed_anal_d = np.zeros(len(K_s),dtype=float)
    fixed_simu_d = np.zeros(len(K_s),dtype=float)
    fixed_anal_e = np.zeros(len(K_s),dtype=float)
    fixed_simu_e = np.zeros(len(K_s),dtype=float)


    for K_s_index in range(len(K_s)):
        ...
        # p_u = p_s
        # p_s = 
        r_c = np.ceil(np.log2(K_s[K_s_index]) + 1) / (2 * np.ceil(np.log2(K_s[K_s_index])))
        ...
        for n in range(M+1):
            ...
            if n > 0:
                ...
                s_r_throughput = 1 - exact_su(K_s[K_s_index], K_u, p_s/(K_s * sigma * alpha_s), r_c, r_s/zeta)
        ...
        fixed_anal_d[K_s_index] = sum_anal_throughput_d
        fixed_anal_e[K_s_index] = sum_anal_throughput_e
        
        print('K_s= ' + str(np.around(K_s[K_s_index],1))
                + ' Anal_D= ' + str(fixed_anal_d[K_s_index])
                + ' Anal_E= ' + str(fixed_anal_e[K_s_index])
                ,end='\n')
        ...
        while(1):
            ...
            H_sr = np.reshape(channel_vector(K_s[K_s_index],K_u,M,1,'rayleigh'),(M,K_u,K_s[K_s_index]))
            ...
            for n in range(M):
                ...
                H_s_u = H_sr[n].T 
                c_hr[n] = zeta * r_c * np.log2(np.abs(1 + p_s / (K_s[K_s_index] * alpha_s * r_c * sigma) \
                    * norm(H_s_u, 'fro') ** 2))
            ...
            if np.any([
                ...
                fixed_simu_d[K_s_index] = fixed_d_counter / simulation_time
                fixed_simu_e[K_s_index] = fixed_e_counter / simulation_time
                print('\n', end='')
                break
    ...
    directory = 'result_txts/ks/fixed/K_u=' + str(K_u) + '_N=' + str(N) + '_M=' + str(M) + '/'
    try:
    ...
    for _ in range(len(file_path)):
        output(file_path[_],K_s,len(K_s),file_results[_])
