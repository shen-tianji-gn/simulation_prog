...
def parser():
    usage = 'Usage: python {} [--N <Number of N>] [--M <Number of M>] [--Ko <Number of Ko>] [--Ke <Number of Ke>] [--Pu <Output Power of Device (dBm)>] [--Period <Simulation period>] [--GPU <Simulation GPU ID>] [--help]'
    ...
    return N,M,K_s,K_e,Pu,period,GPU
...
def main(argv):
    ...
    N, M, K_s, K_e, P_u, simulation_max,GPU = parser()
    ...
    for K_u_index in range(len(K_u)):
        ...
        for n in range(M+1):
            ...
            if n > 0:
                ...
                s_r_throughput = 1 - exact_su(K_s, K_u[K_u_index], p_s/(K_s * sigma * alpha_s), r_c, r_s/zeta)
                r_d_throughput = 1 - outage_ud_fix(K_u[K_u_index],
                                                    r_s,
                                                    zeta,
                                                    sigma_e,
                                                    n,
        ...
        fixed_anal_d[K_u_index] = sum_anal_throughput_d
        fixed_anal_e[K_u_index] = sum_anal_throughput_e
        
        print('K_u= ' + str(np.around(K_u[K_u_index],1))
                + ' Anal_D= ' + str(fixed_anal_d[K_u_index])
                + ' Anal_E= ' + str(fixed_anal_e[K_u_index])
                ,end='\n')
        ...
        while(1):
            ...
            h_e = estimation_error((K_u[K_u_index] * (N - M + relay_counter), 1), sigma_e)
            ...
            if relay_counter == 0:
                ...
                R_d = (1 - zeta) * np.log2(1 + p_u / (K_u[K_u_index] * alpha_ud) * sum_h_rd_2 \
                    / (p_u / (K_u[K_u_index] * alpha_ud) * sum_h_ud_err_2  + sigma))
                # print(R_d)
                R_e = (1 - zeta) * np.log2(1 + p_u / (K_u[K_u_index] * alpha_ue) * sum_H_re_2 \
                    / (p_u / (K_u[K_u_index] * alpha_ue) * sum_H_je_2 + sigma))
    ...
    directory = 'result_txts/ku/fixed/K_s=' + str(K_s) + '_N=' + str(N) + '_M=' + str(M) + '/'
    try:
