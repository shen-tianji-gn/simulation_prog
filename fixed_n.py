...
def parser():
    usage = 'Usage: python {} [--Ku <Number of Ku>] [--Ku <Number of Ku>] [--Ke <Number of Ke>] [--Pu <Output Power of Device (dBm)>] [--Period <Simulation period>] [--GPU <Simulation GPU ID>][--help]'
    ...
    return K_s,K_u,K_e,Pu,period,GPU
...
def main(argv):
    ...
    K_s, K_u, K_e, P_u, simulation_max,GPU = parser()
    ...
    # initial zeros
    fixed_anal_d = np.zeros(len(N),dtype=float)
    fixed_simu_d = np.zeros(len(N),dtype=float)
    fixed_anal_e = np.zeros(len(N),dtype=float)
    fixed_simu_e = np.zeros(len(N),dtype=float)
    ...
    for N_index in range(len(N)):
        ...
        for n in range(M+1):
            ...
            if n > 0:
                ...
                s_r_throughput = 1 - exact_su(K_s, 
                                              K_u, 
                                              p_s/(K_s * sigma * alpha_s), 
                                              r_c, 
                                              r_s/zeta)
                ...
                r_e_throughput = 1 - outage_ue_fix(K_e,
                                                    ...
                                                    zeta)
                # outage D
                sum_anal_throughput_d += \
                    comb(M,n) * s_r_throughput ** n * (1 - s_r_throughput) ** (M-n) * (r_d_throughput)
                
                # outage E
                sum_anal_throughput_e += comb(M,n) * s_r_throughput ** n * (1 - s_r_throughput) ** (M-n) * (r_e_throughput)
                
    
        fixed_anal_d[N_index] = 1 - sum_anal_throughput_d
        fixed_anal_e[N_index] = 1 - sum_anal_throughput_e
        
        print('N= ' + str(np.around(N[N_index],0))
                + ' Anal_D= ' + str(fixed_anal_d[N_index])
                + ' Anal_E= ' + str(fixed_anal_e[N_index])
                ,end='\n')
        ...
        while(1):
            ...
            h_e = estimation_error((K_u * (N[N_index] - M + relay_counter), 1), sigma_e)
            
            H_re = channel_vector(K_u,K_e,relay_counter,0,'rayleigh',GPU_ID=GPU)
            H_je = channel_vector(K_u,K_e,N[N_index]-M,0,'rayleigh',GPU_ID=GPU)
            ...
            if np.any([
                ...
                fixed_simu_d[N_index] = fixed_d_counter / simulation_time
                fixed_simu_e[N_index] = fixed_e_counter / simulation_time
                print('\n', end='')
                break
    ...
    directory = 'result_txts/N/fixed/K_s=' + str(K_s) + '_K_u='+ str(K_u) + '/'
    try:
