...
def main(argv):
    ...
    for N_index in range(len(N)):
        ...
        for P_s_index in range(len(P_s)):
            for P_u_index in range(len(P_u)):
                ...
                p_u = dbm2watt(np.around(P_u[P_u_index],3)) / K_u
    ...
    try:
        os.makedirs('result_txts/opt_N/RicianK=' + str(Rician) + '/fixed/K='\
    ...
    os.chdir('result_txts/opt_N/RicianK=' + str(Rician) + '/fixed/K=' \
        + str(K_u) + 'alpha=' + str(alpha) + '/')
