...
def main(argv):
    ...
    for P_s_index in range(len(P_s)):
        ...
        # fixed_anal_d[P_s_index] = 1 - sum_anal_throughput_d
        # fixed_anal_e[P_s_index] = 1 - sum_anal_throughput_e
        ...
        while(1):
            ...
            for n in range(M):
                ...
                # if K_s >= K_u:
                #     H_sr_2 = np.dot(hermitian(H_sr[n]),H_sr[n])
                # else:
                #     H_sr_2 = np.dot(H_sr[n],hermitian(H_sr[n]))
                u,sigma_svd,v = svd(H_sr[n],full_matrices=True)
                Sigma = np.diag(sigma_svd) ** 2
                
                c_hr[n] = zeta * np.log2(np.abs(det(np.eye(np.min([K_s,K_u])) + p_s / (K_s * alpha_s* sigma) * Sigma)))
    ...
    directory = 'result_txts/ps/fixed_nostbc/K_s=' + str(K_s) + '_K_u='+ str(K_u) + '_N=' + str(N) + '_M=' + str(M) + '/'
    try:
