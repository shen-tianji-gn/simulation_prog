...
def main(argv):
    ...
    # K_u = 2
    # r_s = R_s
    r_s_min = par_lib.R_s_min
    r_s_max = par_lib.R_s_max
    r_s_inst = par_lib.R_s_inst
    r_s = np.around(np.arange(r_s_min,r_s_max+r_s_inst,r_s_inst),1)
    r_c = R_c
    ...
    # initial zeros
    fixed_anal_d = np.zeros(len(r_s),dtype=float)
    fixed_simu_d = np.zeros(len(r_s),dtype=float)
    fixed_anal_e = np.zeros(len(r_s),dtype=float)
    fixed_simu_e = np.zeros(len(r_s),dtype=float)


    for r_s_index in range(len(r_s)):
        ...
        fixed_anal_d[r_s_index] = 1 - sum_anal_throughput_d
        fixed_anal_e[r_s_index] = 1 - sum_anal_throughput_e
        
        print('R_s= ' + str(np.around(r_s[r_s_index],1))
                + ' Anal_D= ' + str(fixed_anal_d[r_s_index])
                + ' Anal_E= ' + str(fixed_anal_e[r_s_index])
                ,end='\n')
        ...
        while(1):
            ...
            H_je = channel_vector(K_u,K_e,N-M,0,'rayleigh',GPU_ID=GPU)
            ...
            # print(R_d)
            # print(r_s)
            if R_d <= r_s[r_s_index]:
                fixed_d_counter += 1
            if R_e <= r_s[r_s_index]:
                fixed_e_counter += 1
            
            
            print('\r' 
                + 'R_s= ' + str((r_s[r_s_index])) 
            ...
            if np.any([
                ...
                fixed_simu_d[r_s_index] = fixed_d_counter / simulation_time
                fixed_simu_e[r_s_index] = fixed_e_counter / simulation_time
                print('\n', end='')
                break
    ...
    directory = 'result_txts/rs/fixed/K_s=' + str(K_s) + '_K_u='+ str(K_u) + '_N=' + str(N) + '_M=' + str(M) + '/'
    try:
    ...
    for _ in range(len(file_path)):
        output(file_path[_],r_s,len(r_s),file_results[_])
