...
def main(argv):
    ...
    for P_s_index in range(len(P_s)):
        ...
        # adapt_anal_d[P_s_index] = sum_anal_throughput_d
        # adapt_anal_e[P_s_index] = sum_anal_throughput_e
        ...
        while(1):
            ...
            H_sr = np.reshape(channel_vector(K_s,K_u,N,1,'rayleigh',GPU_ID=GPU),(N,K_u,K_s))
            ...
            for n in range(N):
                ...
                #     * norm(H_s_u, 'fro') ** 2))     
                c_hr[n] = zeta * np.log2(abs(det(np.eye(min(K_s,K_u)) \
                    + p_s / (K_s * alpha_s * sigma) * np.dot(H_s_u, hermitian(H_s_u)))))
                # print(det(np.eye(min(K_s,K_u)) + p_s / (K_s * alpha_s * sigma) * np.dot(H_s_u, hermitian(H_s_u))))
    ...
    directory = 'result_txts/ps/adapt_fullcsi/K_s=' + str(K_s) + '_K_u='+ str(K_u) + '_N=' + str(N)  + '/'
    try:
