...
def parser():
    usage = 'Usage: python {} [--Ks <Transmitter antenna>][--Ke <Eavesdropper antenna>][--Period <Simulation period>] [--help]'
    argparser = ArgumentParser(usage=usage)
    argparser.add_argument('-u', '--Ku', type=int,
                           required=True,
                           dest='K_u',
                           help='Transmitter antenna')
    argparser.add_argument('-e', '--Ke', type=int,
                           required=True,
                           dest='K_e',
                           help='Eavesdropper antenna')
...
def main(argv):
    ...
    K_u, K_e,simu_max = parser()
    m = np.arange(m_min,m_max+m_inst,m_inst)
    N = 10
    r_s = R_s
    ...
    for m_index in range(len(m)):
        ...
        anal[m_index] = (1 - m[m_index] / (gamma_th * (N - m[m_index]) * K_e)) ** (K_e)
        ...
        while(1):
            ...
            for ke in range(1,K_e +1):
                ...
                sum_channel_signal += abs(sum_unlettered * np.conjugate(sum_unlettered))
            
            for ke in range(K_e):
                for j in range((N-m[m_index])):
                    for k in range(K_u):
                        cha = np.squeeze(channel((1,1)))
    ...
    directory = 'test_results/test2_e/K_u=' + str(np.around(K_u)) + '_K_e=' + str(np.around(K_e)) + '/'
