...
def parser():
    usage = 'Usage: python {} [--N <Number of N>] [--Ko <Number of Ko>] [--help]'
...
def main(argv):
    ...
    zeta = np.around(np.arange(0.2,1,0.02),2)
    ...
    # initial zeros
    # adapt_outage_anal_d = np.zeros(len(zeta),dtype=float)
    # adapt_outage_simu_d = np.zeros(len(zeta),dtype=float)
    # adapt_capa_anal_d = np.zeros(len(zeta),dtype=float)
    # adapt_capa_simu_d = np.zeros(len(zeta),dtype=float)
    # adapt_capa_anal_e_best = np.zeros(len(zeta),dtype=float)
    # adapt_capa_simu_e_best = np.zeros(len(zeta),dtype=float)
    # adapt_capa_anal_e_worst = np.zeros(len(zeta),dtype=float)
    # adapt_capa_simu_e_worst = np.zeros(len(zeta),dtype=float)


    # adapt_secrecy_anal_best = np.zeros(len(zeta),dtype=float)
    # adapt_secrecy_anal_worst = np.zeros(len(zeta),dtype=float)
    # adapt_secrecy_simu_best = np.zeros(len(zeta),dtype=float)
    adapt_secrecy_simu_worst = np.zeros(len(zeta),dtype=float)
    ...
    for zeta_index in range(len(zeta)):
        ...
        P_s_buffer_w = 0
        # P_u_buffer_w = 0
        # adapt_secrecy_buffer_b = 0
        adapt_secrecy_buffer_w = 0
        ...
        for P_s_index in range(len(P_s)):
            ...
            while(1):
                ...
                print('\r' 
                    ...
                    + ' Sec_Simu_W=' + str(adapt_secrecy_buffer_w) \
                ...
                if (any([
                    all([
                        simulation_time >= 100, 
                        adapt_secrecy_capacity_worst_simu <= 0
                    ]),
            ...
            if (adapt_sec_capacity_simu_worst / simulation_max) > adapt_secrecy_buffer_w:
                adapt_secrecy_buffer_w = adapt_sec_capacity_simu_worst / simulation_max
                P_s_buffer_w = P_s[P_s_index] 
                # P_u_buffer_w = P_u[P_u_index]
        # print('\r' + 'P_s_opt= ' + str(P_s_buffer_w)
        #     # + ' Sec_simu_B=' + str(adapt_secrecy_buffer_b)
        #     + ' Sec_simu_W=' + str(adapt_secrecy_buffer_w), end='')
            
        # adapt_outage_simu_d[zeta_index] = adapt_outage_simu_d_counter / simulation_time
        # adapt_capa_simu_d[zeta_index] = adapt_capacity_simu_d / simulation_time
        # adapt_capa_simu_e_best[zeta_index] = adapt_capacity_simu_e_best / simulation_time
        # adapt_capa_simu_e_worst[zeta_index] = adapt_capacity_simu_e_worst / simulation_time
        # adapt_secrecy_simu_best[zeta_index] = adapt_secrecy_buffer_b
        adapt_secrecy_simu_worst[zeta_index] = adapt_secrecy_buffer_w
        print('\n', end='')
        
    # make dir if not exist
    try:
        os.makedirs('result_txts/zeta/RicianK=' + str(Rician) + '/adapt_op/K=' + str(K_u) + '_N=' + str(N) + '/')
    except FileExistsError:
        pass


    # result output to file
    os.chdir('result_txts/zeta/RicianK=' + str(Rician) + '/adapt_op/K=' + str(K_u) + '_N=' + str(N) + '/')
    ...
    for _ in range(len(file_path)):
        output(file_path[_],zeta,zeta_index,file_results[_])
