...
def main(argv):
    ...
    for P_s_index in range(len(P_s)):
        ...
        while(1):
            ...
            adapt_capacity_simu_d += float(R_d)
            adapt_capacity_simu_e += float(R_e)
    ...
    dir = 'result_txts/ps/RicianK=' + str(Rician) + '/adapt_qr/K_s=' + str(K_s) + '_K_u='+ str(K_u) + '_N=' + str(N) + '/'
    try:
        os.makedirs(dir)
